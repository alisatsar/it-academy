...
shader_gl_es20* shader04 = nullptr;
...
void om::engine::render(const vbo& buff, texture* tex, const om::mat2x3& m,
		const glm::mat4& camera_matrix)
{
    shader04->use();
    texture_gl_es20* texture = static_cast<texture_gl_es20*>(tex);
    texture->bind();
    shader04->set_uniform("s_texture", texture);
    shader04->set_uniform("u_matrix", m);
    shader04->set_uniform("u_camera_matrix", camera_matrix);
...

    {
        ...
        shader04 = new shader_gl_es20(
                R"(
                        ...
                        v_tex_coord = a_tex_coord;
                        v_color = a_color;
                        vec3 pos1 = vec3(a_position, 1.0) * u_matrix;
 						vec4 pos2 = vec4(pos1, 1.0)	* u_camera_matrix;
                        gl_Position = vec4(pos2);
                        }
                        )",
                R"(
                        varying vec2 v_tex_coord;
