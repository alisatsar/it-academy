...
void initAudio(void)
{
    ...
    /* want.userdata = new; */
    if((gDevice->device = SDL_OpenAudioDevice(NULL, 0, &(gDevice->want), NULL, SDL_AUDIO_ALLOW_FREQUENCY_CHANGE)) == 0)
    {
...
Audio * createAudio(const char * filename, uint8_t loop, int volume)
{
    ...
    if(SDL_LoadWAV(filename, &(new->audio), &(new->buffer_true), &(new->length_true)) == NULL)
    {
