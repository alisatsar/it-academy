...
public:
	...
	///check if the hero jumped on the subject
	bool test_collision_up(collision_box* box);
	~hero_controller();
};
...
bool hero_controller::test_collision_up(collision_box* box)
{
	collision_box c = this->get_collision_box();
	float delta = fabs(box->v0.y - c.v1.y);
	return  delta >= 0.0f && delta <= 0.05f;
}
